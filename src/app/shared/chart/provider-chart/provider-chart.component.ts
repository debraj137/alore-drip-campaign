...
@Component({
  ...
  styleUrls: ['./provider-chart.component.scss']
})
export class ProviderChartComponent implements OnInit {
  ...
  ngOnChanges(change: SimpleChanges){
    ...
    switch(this.graphIndex){
      case 0:
        this.api = 'getGmailReplyReport'
        this.getProviderData(startDate, endDate);
        return;
      
      case 1:
        this.api = 'getOutlookReplyReport'
        this.getProviderData(startDate, endDate);
        return;

      case 2: 
        this.api = 'getOtherReplyReport'
        this.getProviderData(startDate, endDate);
        return;
  ...
  getProviderData(startDate: Date, endDate: Date) {
    ...
    this.analyticsService.getProviderReplyRates(this.api, payload).subscribe(
      (resp: any) => {
        this.loader = false
        if (resp.responseCodeJson.code === 200) { 
          this.isGraphDataEmpty = !resp.object.data?.length
          this.totalData = resp.object.openedMail
          const percentage = Number(resp.object.totalReplyRate)
          ...
          resp.object.data.forEach((obj: any, index: number) => {
            ...
            this.barChartData[0].data.push(dailyData.opened)
            this.barChartData[1].data.push(dailyData.replied)
