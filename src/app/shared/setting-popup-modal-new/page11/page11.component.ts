...
import { Component, ElementRef, Input, Inject,Output, OnInit,EventEmitter, ViewChild } from '@angular/core';
...
export class Page11Component implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<Page11Component>,
  ...
  getTagData() {
    this.tagService.getTagList(this.campId).subscribe(
      (resp : any) => {
  ...
  submitTagData() {
    ...
    this.tagService.addTags(payload,this.campId).subscribe(
      (resp : any) => {
        if (resp.responseCodeJson.code === 200) {
